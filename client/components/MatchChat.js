...
export default function MatchChat({profileId, matchId}) {
  ...
  return (
    <div>
      ...
      <input className='border-solid border-2 border-indigo-600 leading-9 w-9/12' value={newMessage} onChange={(e) => setNewMessage(e.target.value)}/>
      <button className="border-solid border-2 rounded ml-4 p-2 border-indigo-600 active:bg-indigo-500 hover:bg-indigo-300" onClick={sendMessage}>Send</button>
    </div>
  );
}
